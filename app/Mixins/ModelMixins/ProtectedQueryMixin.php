...
trait ProtectedQueryMixin
{
    ...
    public function appliedMullipleFilter($querySet, $filter)
    {
        ...
        foreach ($queryFilters as $qFilter) {
            ...
            } else if(StringAlgorithm::firstPatternMatching($qFilter['key'], '__gte')) {
                $keyMap = explode('__gte', $qFilter['key']);
                $key = $keyMap[0];
                $key = $this->joinQuery($key, $this);
                $querySet->where($key, '>=', $qFilter['value']);
            }
            else if(StringAlgorithm::firstPatternMatching($qFilter['key'], '__lte')) {
            ...
            else if ($this->startsWith($qFilter['key'], 'notin~')) {
                $len = strlen('notin~');
                $key = substr($qFilter['key'], $len);
                $key = $this->joinQuery($key, $this);
                $searchData = explode("--",$qFilter['value']);
            ...
            else if ($this->startsWith($qFilter['key'], 'or~')) {
                $len = strlen('or~');
                $key = substr($qFilter['key'], $len);
                $key = $this->joinQuery($key, $this);
                $searchData = explode("--",$qFilter['value']);
            ...
            else if ($this->startsWith($qFilter['key'], '&|')) {
