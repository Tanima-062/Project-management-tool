...
trait ProtectedVersionable
{
     ...
     * @return className
     */
    public function getVersionableClassName()
    {
    ...
    public function render_total_versions()
    {
        return Version::where('versionable_id', $this->id)
            ->where('versionable_type', $this->getVersionableClassName())->orderBy('id', 'desc')->limit(10)->pluck('version')->toArray();
    }

    /**
     * Get a model based on the version id
     *
     * @param $versionable_id
     *
     * @return $version_number
     */
    public function getNewVersionNumber($versionable_id)
    {
        $versionObject = Version::where('versionable_id', $versionable_id)->orderBy('created_at', 'desc')
            ->orderBy('id', 'desc')
            ->where('versionable_type', $this->getVersionableClassName())->first();
        return $versionObject ? intval($versionObject->version) + 1 : 1;
    }
    ...
    /**
     * Save a old version if data first time created by seed or script.
     * @return void
     */
    protected function oldVersionableSave($resource)
    {
         // Save a new version
        $versionObject = Version::where('versionable_id', $resource->id)
            ->where('versionable_type', $this->getVersionableClassName())->first();
        if (empty($versionObject) && !$versionObject && $this->versioningEnabled === true) {
            $class                     = $this->getVersionClass();
            $version                   = new $class();
            $version->version          = $this->getNewVersionNumber($this->getKey());
            $version->versionable_id   = $resource->id;
            $version->versionable_type = get_class($this);
            $version->user_id          = $this->getAuthUserId();
            $version->model_data       = json_encode($resource->getAttributes());
    ...
    protected function versionablePostSave()
    {
        ...
        if (
            ...
            $version->versionable_type = get_class($this);
            $version->user_id          = $this->getAuthUserId();
            $version->model_data       = json_encode($this->getAttributes());
