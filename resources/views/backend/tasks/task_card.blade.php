...
<body class="task-card-body">
	<div class="app">
		<div class="lists">
            <div class="list">
                <p class="kanban-todo-col sticky">Accepted (<span class="total-accepted">{{$total_accepted}}</span>)</p>
                @foreach($tasks_for_kanban as $task)
                    @php $status = \App\Modals\Task::getKanbanTaskStatus($task); @endphp
                        @if ($status == 0)
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        {{-- <div class="list-item-item">
                                            <div style="height: 10px; width: 13px; border-radius: 50%; background-color:#FFCE00; margin-right: 10px"></div>
                                            <p style="color:#909190">{{$task->title}}</p>

                                            <div style="display:flex; height:20px; width:100%; align-items:center; justify-content:flex-end">
                                                <i style="color:#AFAFAF" class="fa fa-paperclip" aria-hidden="true"></i>
                                                <b style="margin-left:5px; color:#AFAFAF">2</b>
                                            </div>
                                        ...
                                        <div class="row p-0">
                                            <div class="col-md-12 kanban-card-header-left pr-0">
                                                ...
                                                <a href="{{route('tasks.show',$task->id)}}" style="color: #ec008c;text-decoration:none;" class="kanban-hover">
                                                    <p class="kanban-card-header-para" style="color:#282828;">
                                                        {{-- {{$task->title}} --}}
                                                        &nbsp;&nbsp;{{ empty($task->title) ? '' : (strlen($task->title) > 30 ? substr($task->title, 0, 30).'...' : $task->title) }}
                                                    </p>
                                                </a>
                                            </div>
                                            {{-- <div class="col-lg-3 col-md-4 kanban-card-header-right pl-0">
                                                <i style="color:#AFAFAF" class="fas fa-paperclip" aria-hidden="true"></i>
                                                <b style="margin-left:5px; color:#AFAFAF">2</b>
                                            </div> --}}
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="list-item-item todo-description">
                                            {{-- {{$task->description}} --}}
                                            {!! empty($task->description) ? '' : (strlen($task->description) > 115 ? substr($task->description, 0, 115).'...' : $task->description) !!}
                                        </div>
                                    </div>
                                    <div class="col-md-12">
                                        <div class="row">
                                            <div class="col-md-8">
                                                <div class="kanban-card-footer" >
                                                    ...
                                                    @if($task->assign_to == 'team')
                                                        @foreach($task->memberTasks as $key=>$member_task)
                                                            @if(!empty($member_task->user->image) && (file_exists(public_path('backend/uploads/profile_images/'.$member_task->user->id.'/'.$member_task->user->image))))
                                                                <img src="{{asset('backend/uploads/profile_images/'.$member_task->user->id.'/'.$member_task->user->image)}}" alt="profile"  class="team-card-member-image"/>
                                                            @else
                                                        ...
                                                        @endforeach
                                                        @if(count($task->memberTasks)>4)
                                                            <div  class="team-card-member-image team-card-member-image-addition-count">+{{count($task->memberTasks) - 4}}</div>
                                                        @endif
                                                    @elseif($task->assign_to == 'individual')
                                                        @foreach($task->assignees as $key=>$assignee)
                                                            @if(!empty($assignee->user->image) && (file_exists(public_path('backend/uploads/profile_images/'.$assignee->user->id.'/'.$assignee->user->image))))
                                                                <img src="{{asset('backend/uploads/profile_images/'.$assignee->user->id.'/'.$assignee->user->image)}}" alt="profile"  class="team-card-member-image"/>
                                                            @else
                                                                <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" class="kanban-card-member-image"><path d="M0 0h24v24H0z" fill="none"/><path d="M12 2C6.48 2 2 6.48 2 12s4.48 10 10 10 10-4.48 10-10S17.52 2 12 2zm0 3c1.66 0 3 1.34 3 3s-1.34 3-3 3-3-1.34-3-3 1.34-3 3-3zm0 14.2c-2.5 0-4.71-1.28-6-3.22.03-1.99 4-3.08 6-3.08 1.99 0 5.97 1.09 6 3.08-1.29 1.94-3.5 3.22-6 3.22z"/></svg>
                                                            @endif 
                                                            @if($key == 3)  @break;  @endif  
                                                        @endforeach
                                                        @if(count($task->assignees)>4)
                                                            <div  class="team-card-member-image team-card-member-image-addition-count">+{{count($task->assignees) - 4}}</div>
                                                        @endif 
                                                    @endif
                                                    </div>
                                                </div>
			...
			<div class="list">
                <b class="kanban-inprogress-col sticky">In Progress (<span class="total-inProgress">{{$total_inProgress}}</span>)</b>
                @foreach($tasks_for_kanban as $task)
                    @php $status = \App\Modals\Task::getKanbanTaskStatus($task); @endphp
                        @if ($status == 1)
                            <div class="list-item card py-3" draggable="true">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-md-12">
                                        {{-- <div class="list-item-item">
                                            <div style="height: 10px; width: 13px; border-radius: 50%; background-color:#095CF8; margin-right: 10px"></div>
                                            <b style="color:#909190">{{$task->title}}</b>
                                            <div style="display:flex; height:20px; width:100%; align-items:center; justify-content:flex-end">
                                                <i style="color:#AFAFAF" class="fa fa-paperclip" aria-hidden="true"></i>
                                                <b style="margin-left:5px; color:#AFAFAF">2</b>
                                            </div>
                                    ...
                                    <div class="col-md-12">
                                        <div class="row">
                                            ...
                                            {{-- <div style="font-size:10px;">{{empty($task->end_date) ? '' : date('d M, Y', strtotime($task->end_date))}}</div> --}}
                                            @if($task->end_date < date('Y-m-d'))
			...
			<div class="list">
                <p class="kanban-completed-col sticky">Completed (<span class="total-completed">{{$total_completed}}</span>)</p>
                @foreach($tasks_for_kanban as $task)
                    @php $status = \App\Modals\Task::getKanbanTaskStatus($task); @endphp
                        @if ($status == 2)
                            <div class="list-item card py-3" draggable="true">
                                ...
                                <div class="row">
                                    <div class="col-md-12">
                                        {{-- <div class="list-item-item">
                                            <div style="height: 10px; width: 14px; border-radius: 50%; background-color:#12B002; margin-right: 10px"></div>
                                            <b style="color:#909190">{{$task->title}}</b>
                                            <div style="display:flex; height:20px; width:100%; align-items:center; justify-content:flex-end">
                                                <i style="color:#AFAFAF" class="fa fa-paperclip" aria-hidden="true"></i>
                                                <b style="margin-left:5px; color:#AFAFAF">2</b>
                                    ...
                                    <div class="row">
                                        <div class="col-md-12">
                                            <div class="kanban-card-footer">
                                                ...
                                                {{-- <div style="height:10px; width:30%; display:flex; align-items:center; justify-content:flex-end">
                                                    <a href="https://www.facebook.com/"><i style="color:#AFAFAF" class="fa fa-ellipsis-h" aria-hidden="true"></i></a>
                                                </div> --}}
                                            </div>
                                        </div>
                                    </div>
