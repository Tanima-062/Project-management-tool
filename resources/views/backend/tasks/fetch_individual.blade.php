                            ...
                            @if(isset($member->user->name) && ($member->user->status > 0) && ($member->user->id != \Illuminate\Support\Facades\Auth::user()['id']))
                                <div class="{{$team->id}}-all">
                                    <label class="container" for="{{$team->id}}{{$member->user_id}}">
                                        ...
                                        <input type="checkbox"  value="{{$member->team_id}}" style="display: none">
                                        <input type="checkbox" id="{{$team->id}}{{$member->user_id}}" class="checkbox checkbox-member individual_values" value="{{$member->user_id}}" >
                                        <span class="checkmark"></span>
                                    </label>
                                </div>
                            @endif
