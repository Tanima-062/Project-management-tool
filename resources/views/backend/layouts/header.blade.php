...
<nav class="navbar col-lg-12 col-12 p-0 fixed-top d-flex flex-row">
    <div class="text-center navbar-brand-wrapper d-flex align-items-center justify-content-center" style="background-color:#009877;">
        <a class="navbar-brand brand-logo" href="{{route('dashboard')}}"><img src="{!!URL::to('public/backend/assets/img/logo_stl.png')!!}" class="mr-2 img-fluid" alt="logo"/></a>
        <a class="navbar-brand nav-logo" href="{{route('dashboard')}}"><img src="{!!URL::to('public/backend/assets/img/stl_icon.png')!!}" class="mr-2 img-fluid" alt="logo"/></a>
    </div>
    ...
    <div class="navbar-menu-wrapper d-flex align-items-center justify-content-end" style="background-color:#009877;">
        ...
        <ul class="navbar-nav navbar-nav-right">
            ...
            <li class="nav-item header-create-button">
                <a class="btn-hover" href="{{route('tasks.create')}}">
                    <button class="Rectangle-btn">
                        <i class="far fa-plus-square"></i>
                        NEW TASK
                    </button>
                </a>
                <a class="btn-hover" href="{{route('myRequestTasks.create')}}">
                    <button class="Rectangle-btn" style="width:150px;">
                        {{-- <img src="{{asset('backend/assets/img/new-task-icon.png')}}" class="New-Task-Plus-Icon"> --}}
                        <i class="far fa-plus-square"></i>
                        PROPOSE TASK
                    </button>
                </a>
            ...
            <a class="nav-item nav-profile" href="{{route('users.show',\Illuminate\Support\Facades\Auth::user()['id'])}}">
                @if(!empty(\Illuminate\Support\Facades\Auth::user()['image']) && (file_exists(public_path('backend/uploads/profile_images/'.\Illuminate\Support\Facades\Auth::user()['id'].'/'.\Illuminate\Support\Facades\Auth::user()['image']))))
...
<script type="text/javascript">
    ...
    channel.bind(JSON.parse("{{ json_encode(Auth::user()->id) }}")+"_user_notification", function(data) {
        console.log("work");
        notificationsCount += 1;
        unShowedNotification += 1;
        var existingNotifications = notifications.html();
