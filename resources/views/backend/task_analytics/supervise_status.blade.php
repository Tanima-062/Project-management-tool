    ...
    <div class="card directSuperviseStatusCard">
            ...
            <div class="row">
                ...
                <div class="col-md-5 col-sm-9 overall-task-quantity">
                            ...
                            <div class="row">
                                <div class="col pl-sm-0">
                                    ...
                                    <h3 class="task-quantity-number Members">{{$memberCount}}</h3>
                                </div>
                                <div class="col pl-sm-0">
                                    <h4 class="task-quantity-title">Tasks</h4>
                                    <h3 class="task-quantity-number Tasks">{{$overall_task_count['tasks']}}</h3>
                                </div>
                                <div class="col pl-sm-0">
                                    <h4 class="task-quantity-title">Pending</h4>
                                    <h3 class="task-quantity-number Pending">{{$overall_task_count['pending']}}</h3>
                                </div>
                                <div class="col pl-sm-0">
                                    <h4 class="task-quantity-title">Accepted</h4>
                                    <h3 class="task-quantity-number Accepted">{{$overall_task_count['accepted']}}</h3>
                                </div>
                                <div class="col pl-sm-0">
                                    <h4 class="task-quantity-title">In Progress</h4>
                                    <h3 class="task-quantity-number In-Progress">{{$overall_task_count['inProgress']}}</h3>
                                </div>
                            </div>
                ...
                <div class="col-md-3 col-sm-9 overall-task-quantity">
                    <div class="row">
                        ...
                        <div class="col-4 p-md-0">
                            ...
                            <h3 class="task-quantity-number Completed">{{$overall_task_count['completed']}}</h3>
                        </div>
                        <div class="col-4 pl-sm-0">
                            <h4 class="task-quantity-title">Rejected</h4>
                            <h3 class="task-quantity-number Rejected">{{$totalRejected}}</h3>
                        </div>
                        <div class="col-4 pl-sm-0">
                            <h4 class="task-quantity-title">Overdue</h4>
                            <h3 class="task-quantity-number overdue">{{$overall_task_count['overdue']}}</h3>
                        </div>            
                ...
                <div class="col-md-2 col-sm-3 program-wise-member-images">
                    <div class="kanban-card-footer task-card-body" >
                        <div class="d-flex">
                            @foreach($totalMembers as $key => $member)
                                @if(isset($member))
                                    @if(!empty($member->image) && (file_exists(public_path('backend/uploads/profile_images/'.$member->id.'/'.$member->image))))
                                        <img src="{!! URL::to('public/backend/uploads/profile_images/'.$member->id.'/'.$member->image)!!}" class="team-card-member-image">
                                    @else
                            ...
                            @endforeach
                            @if(count($totalMembers)>4)
                                <div  class="team-card-member-image team-card-member-image-addition-count">+{{count($totalMembers) - 4}}</div>
                            @endif                                                         
                        </div>
                    </div>
                </div>
        ...
        <div class="row DataTableBox">
            ...
            <div class="table-responsive" style="margin-bottom: 20px;">
                @include('backend.task_analytics.member')
            </div>
            </div>
        </div>
    </div>