                ...
                <div class="row DataTableBox">
                    <div class="row role-list-table-subHeader">
                        ...
                        <div class="col-sm-7 subHeader-col-2">
                            ...
                            @if(\App\Modals\User::hasSpecificPermission(\Illuminate\Support\Facades\Auth::user(),'role.create'))
                            <a class="btn-hover" href="{{route('roles.create')}}">
                            ...
                            @foreach($roles as $role)
                                @if(isset($role))
                                    <tr>
                                        ...
                                        <td>
                                            @if(\App\Modals\User::hasSpecificPermission(\Illuminate\Support\Facades\Auth::user(),'role.edit'))
                                               <a class="Rectangle-Edit btn-hover2" style="text-decoration: none;" href="{{route('roles.edit', $role->id)}}"><i class="fa fa-pencil" style="margin-right: 5px;"></i>Edit</a>
                                            @endif
                                            @if(\App\Modals\User::hasSpecificPermission(\Illuminate\Support\Facades\Auth::user(),'role.delete'))
                                                <a class="Rectangle-Delete btn-hover2" style="text-decoration: none;" href="{{ route('roles.destroy', $role->id) }}"
                                                   onclick="deleteData('delete-form-{{$role->id}}')"><i class="fa fa-trash"></i>
                                                    Delete
                                                </a>

                                                <form id="delete-form-{{$role->id}}" action="{{ route('roles.destroy', $role->id) }}" method="POST" class="d-none" style="display: none">
                                                    @method('DELETE')
                                                    @csrf
                                                </form>
                                            @endif
