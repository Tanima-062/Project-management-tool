            ...
            <div class="content-wrapper">
                        ...
                        <div class="col-md-12">
                            ...
                            <div class="media" style="align-items: center;">
                                ...
                                @if(isset($task->requestFrom->image) && file_exists(public_path('backend/uploads/profile_images/'.$task->requestFrom->id.'/'.$task->requestFrom->image)))
                                    <img src="{{asset('backend/uploads/profile_images/'.$task->requestFrom->id.'/'.$task->requestFrom->image)}}" height="55" width="55" style="border-radius: 50%">
                                @else
                                ...
                                <div class="media-body">&nbsp;&nbsp;{{empty($task->requestFrom->name) ? '' : $task->requestFrom->name}}</div>
                            </div>
                            <br>
                        </div>
                    </div>    
                ...
                <div class="CreateTaskBox card-body">
                    <div class="row">
                        ...
                        <div class="col-md-12">
                            <form method="POST" action="{{route('otherRequestTasks.update',$task->id)}}" enctype="multipart/form-data" id="taskForm">
                                @method('PUT')   
                                @csrf
                                            ...
                                            <div class="media comment">
                                                <div class="media-left">
                                                    @if(isset($comment->user->image) && file_exists(public_path('backend/uploads/profile_images/'.$comment->user->id.'/'.$comment->user->image)))
                                                        <img src="{{asset('backend/uploads/profile_images/'.$comment->user->id.'/'.$comment->user->image)}}" height="40" width="40" style="border-radius: 50%">
                                                        &nbsp;&nbsp;
                                                    @else
                    ...
                    <div class="modal-dialog" role="document">
                        <div class="modal-content">
                            <div class="modal-body">
                                <form method="POST" action="{{route('otherRequestTasks.reject')}}" id="rejectForm">         
                                    @csrf    
                                    <div class="form-group">
                                        <label for="reason"><b>Reason For</b></label>
                                        <select id="reason" name="reason" class="form-control">
                                            @foreach($reasons as $reason)
                                                <option value="{{$reason->name}}" {{ $reason->name == 'others' ? 'Selected' : ''}}>{{empty($reason->displayName) ? '' : $reason->displayName}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="row">
