        ...
        <div class="main-panel role-list-main-panel">
            <div class="content-wrapper">
                ...
                <div class="row DataTableBox">
                    ...
                    <form action="{{route('notification.module.store')}}" method="POST">
                        @csrf
                        <div class="table-responsive" style="margin-bottom: 20px;">
                            <table class="table">
                                ...
                                <tbody>
                                @foreach($notification_modules as $event)
                                    <tr>
                                        <td>{{$loop->index+1}}</td>
                                        <td>{{$event->displayName}}</td>
                                        <td>
                                            <div class="form-check">
                                                <input type="checkbox" class="checkbox" name="emails[]" id="email{{$event->id}}" value="{{$event->event}}" {{ ($event->email == 1) ? 'checked' : '' }}>
                                                <label class="form-check-label" for="email{{$event->id}}"></label>
                                            </div>
                                        </td>
                                        <td>
                                            <div class="form-check">
                                                <input type="checkbox" class="checkbox" name="in_app[]" id="in_app{{$event->id}}" value="{{$event->event}}" {{ ($event->in_app == 1) ? 'checked' : '' }}>
                                                <label class="form-check-label" for="in_app{{$event->id}}"></label>
                                            </div>
                                        </td>
                                    </tr>
                                @endforeach
