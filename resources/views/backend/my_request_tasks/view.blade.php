                        ...
                        <div class="row" style="margin-top: 20px;">
                            ...
                            @foreach($task->attachments as $attachment)
                                <div>
                                    ...
                                    <a href="{{route('request.file.download',['taskId' => $task->id,'fileName' => $attachment->file_name])}}">{{empty($attachment->file_name) ? '' : $attachment->file_name}}</a>
                                </div>
                            @endforeach
                        </div>
                    @endif
                                            ...
                                            <div class="media-body">
                                                <div class="comment-detail-{{$comment->id}}">
                                                    <p>
                                                        ...
                                                        @endif
                                                        <form id="delete-comment-form-{{$comment->id}}" action="{{route('requestTask.comment.delete', $comment->id)}}" method="POST" class="d-none" style="display: none">
                                                        @csrf
                                                        </form>
                                                    </p>
                                                    <p>{!! $comment->text !!}</p>
                                                    @if(count($comment->attachments) > 0)
                                                        @foreach($comment->attachments as $attachment)
                                                            <div class="hand-icon-added">
                                                                ...
                                                                <a href="{{route('requestTask.download.commentFile',['commentId' => $comment->id,'fileName' => $attachment->file_name])}}">{{empty($attachment->file_name) ? '' : $attachment->file_name}}</a>
                                                            </div>
                                                        @endforeach
                                                    @endif
                                                </div>
                                                <form method="POST" action="{{route('requestTask.comment.edit', $comment->id)}}" class="comment-form comment-edit-form-{{$comment->id}}" id="comment-edit-form-{{$comment->id}}" enctype="multipart/form-data" style="display:none;">
                                                    @csrf
                                                ...
                                                <form method="POST" class="comment-form" action="{{route('requestTask.reply',$comment->id)}}" id="comment-{{$comment->id}}" style="display: none;" enctype="multipart/form-data">
                                                    @csrf
                                                                ...
                                                                @if(count($reply->attachments) > 0)
                                                                    @foreach($reply->attachments as $attachment)
                                                                        <div class="hand-icon-added">
                                                                            <!-- <input type="checkbox" disabled> -->
                                                                            <i class="fas fa-hand-point-right"></i>
                                                                            <a href="{{route('requestTask.download.replyFile',['replyId' => $reply->id,'fileName' => $attachment->file_name])}}">{{empty($attachment->file_name) ? '' : $attachment->file_name}}</a>
                                                                        </div>
                                                                    @endforeach
                                                                @endif
